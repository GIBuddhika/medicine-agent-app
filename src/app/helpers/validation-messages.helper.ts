...
export class ValidationMessagesHelper {
    ...
    */
            ...
            if (control.status == "INVALID" && element) {
                for (let error in control.errors) {
                    switch (error) {
                        ...
                        case 'pattern':
                            let errorPattern = element.getAttribute('error-pattern');
    ...
    private showError(element, id, errorText) {
        ...
        element.insertAdjacentHTML('afterend', "<p class='invalid-" + id + " invalid-input'>" + errorText + "</p>");
    }
}
